...
contract MARSWAPFARM is masterChef, ERC20('MARSWAP FARM', 'MSWAPF') {
    ...
    constructor(address _router, address _MarketingWallet, uint256 initialSupply)  {
       devFee = 400;  // 100 = 1%
       burnFee = 0; // 100 = 1%
       swapperFee = 25; // 100 = 1%
    ...
    function setBurnRouter(address _burnRouter) external onlyOwner {
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if(automatedMarketMakerPairs[to] || automatedMarketMakerPairs[from]) {
                uint256 extraFee;
                if(devFee >0 || burnFee >0) extraFee = (amount * (devFee + burnFee)) / 10000;
   ...
   function useSwapper(uint256 amount) private lockSwapper {
    ...
    function SwapFees() private lockTheSwap {
          ...
          address[] memory path = new address[](2);
            ...
            path1[0] = ETH;
            path1[1] = burnToken;
            uint256 preBurn = ERC20(burnToken).balanceOf(dead);
            ...
            uint256 postBurn = ERC20(burnToken).balanceOf(dead);
            totalBurned += (postBurn - preBurn);
            }
            TransferHelper.safeTransferETH(marketingWallet, address(this).balance);
        ...
        function removeBONE() external onlyOwner {
            TransferHelper.safeTransferETH(msg.sender, address(this).balance);
        }

        function removeTokens(address _tokenAddress) external onlyOwner {
            require(_tokenAddress != address(this), "Cannot withdrawl MSWAP");
