...
contract TokenPriceCheck  {  
    ...
    constructor(address _eth, address _usdt, address[] memory _factories) {
        ...
        uint256 d = Token(_usdt).decimals();
        usdtDecimalConversion = 18 - d;
        owner = msg.sender;
    }

    function changeUSDT(address newUSD) external {
        require(msg.sender == owner, "Not Owner");
        usdt = newUSD;
        uint256 d = Token(newUSD).decimals();
        usdtDecimalConversion = 18 - d;
    }
...
function getBNBPrice(address startFactory) public view returns (uint256 price) {
    ...
    try Factory(startFactory).getPair(bnb, usdt) {
        ...
        if (r0 != 0 && r1 != 0) {
            if (token0 == bnb) {
                price = ((r1 * 10**usdtDecimalConversion) * usPrice) / r0;
            } else {
                price = ((r0 * 10**usdtDecimalConversion) * usPrice) / r1;
            }
...
if(pair == zeroA) {
    ...
    try LPToken(pair).getReserves() {
        ...
        if(token0 == token) {
            num = token1 == usdt ? r1*10**usdtDecimalConversion : r1;
            den = r0;
            usd = usPrice;
        } else {
            num = token0 == usdt ? r0*10**usdtDecimalConversion : r0;
            den = r1;
            usd = usPrice;
