...
contract SmartNFTPoolRenewable is IERC721Receiver ,Ownable, ReentrancyGuard {
    ...
    uint256 public monthlyFee =  25000000000000000;    
   ...
   // Subscription
    ...
    function setmonthlyFees(uint256 _monthlyFee) external onlyOwner {
        monthlyFee = _monthlyFee;
    }
    ...
    */
    ...
    function increaseAPR() public onlySub {
     ...
     function getTokenAndFeeInfo(uint256 _howManyDays) public view returns(uint256 _tokenAmount, uint256 _feeToPay) {
        RoundInfo storage round = roundInfo[currentRound];
        _tokenAmount = round.rewardPerBlock * ( _howManyDays * 86400);
    ...
    function ExtendPool(uint256 _howManyDays) public payable onlySub {
        ...
        if(msg.sender != owner()) {
            (bool success,) = treasury.call{value:feeToPay}(new bytes(0));
            require(success, 'TransferHelper: ETH_TRANSFER_FAILED');
        }
          
        _updatePool();
        uint256 totalNewReward = checkTotalNewRewards();
        uint256 newRewardsToUse = (_howManyDays * 86400) * round.rewardPerBlock;
    ...
    function startNewPoolV2(uint256 _startInDays, uint256 _poolLengthDays , uint256 _tokenAmount) external payable onlySub {
        ...
        rewardToken.transferFrom(address(msg.sender), (address(this)), _tokenAmount);
        // start the new pool
        startNewPoolOrRound(_startInDays, _poolLengthDays);
    }

    function startNewPoolOrRound( uint256 _startInDays, uint256 _poolLengthDays ) public payable onlySub {
        RoundInfo storage round = roundInfo[currentRound];
...
contract NFTPoolFactory is Ownable {
    ...
    function createNFTPool(IFarmageddonNFT nftContract, IBEP20 partnerToken) external payable {
          (bool success,) = treasury.call{value:subFee}(new bytes(0));
        require(success, 'TransferHelper: ETH_TRANSFER_FAILED');
