...
import {IterableMapping} from "../Dex/IterableMapping.sol";
...
contract DividendToken is ERC20, Ownable {
    ...
    constructor(
        ...
        address[5] memory addrs, // reward, router, marketing wallet, base Token , dividend tracker
        ...
        DividendTokenDividendTracker(dividendTracker).excludeFromDividends(_realOwner);
        ...
        excludeFromFees(_realOwner, true);
        ...
        isExcludedFromMaxTransactionAmount[_realOwner]=true;     
        _mint(_realOwner, totalSupply_);
     ...
     function updateUniswapV2Pair(address _baseTokenForPair) external onlyOwner
    {
        ...
        uniswapV2Pair = IUniswapV2Factory(IUniswapV2Router02(uniswapV2Router).factory()).getPair(address(this),baseTokenForPair);
        if(uniswapV2Pair == address(0)){
            uniswapV2Pair = IUniswapV2Factory(IUniswapV2Router02(uniswapV2Router).factory()).createPair(address(this), baseTokenForPair);
        }
        _setAutomatedMarketMakerPair(uniswapV2Pair, true);
    }
    ...
    function swapTokensForBaseToken(uint256 tokenAmount) private {
        ...
        if (path[1] == IUniswapV2Router02(uniswapV2Router).WETH()){
            ...
            _approve(address(this), address(swapCaller), tokenAmount);
            swapCaller.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                    uniswapV2Router,
                    tokenAmount,
                    0, // accept any amount of BaseToken
                    path,
