...
/*
    Farmageddon's NFT Collection Contract
    https://farmageddon.farm
    A family Farms better together.
*/
...
contract MarsRandomNFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(
    ...
    if(msg.value > 0){
       (bool sent, ) = payable(_treasury).call{value: msg.value}("");
       require(sent, "fail to transfer fee"); 
    }
    setBaseURI(_initBaseURI);
    baseExtension = _initBaseEXT;
    treasury = _treasury;
    costBNB = _costBNB;
    ...
    FGAdmin = _treasury;
    require(_names.length == _maxs.length,"Must have same amount of names and rarities");
    uint256 maxCheck = 0;
    for(uint i=0; i<_names.length;i++){
        imageInfo.push(imageData(_names[i], _maxs[i]*10, _maxs[i]));
        maxCheck += _maxs[i];
    }
    require(maxCheck == maxSupply, "Set max's dont equal Max Supply");
  }
  ...
  function _baseURI() internal view virtual override returns (string memory) {
   ...
   function changeExtension(string memory _ext) external onlySub {
    baseExtension = _ext;
  }
  ...
  function ownerMint(address _to, uint256 _mintAmount) external onlySub returns (uint256[] memory ids) {
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    return bytes(currentBaseURI).length > 0
        ? string(abi.encodePacked(currentBaseURI, NFTInfo[tokenId-1].Name, baseExtension))
        : "";
  }
  // RNG
  function rng() internal view returns(uint256) {
    uint256 seed = block.timestamp + block.prevrandao + block.gaslimit + totalSupply() + gasleft(); 
    uint256 value = uint256(keccak256(abi.encodePacked(seed))) % getTotalRarityLeft();
    return value;
  }
  
    function getTotalRarityLeft() internal view returns(uint256 total){
        total = 0;
        for(uint i=0; i<imageInfo.length; i++){
            imageData memory image = imageInfo[i];
            if(amountMinted[i] < image.Max) total +=  image.Rarity;
        }
    }


  // randomly pick a image
  function setNFTInfo() internal {    
        uint256 value = rng();
        uint256 setting = getTotalRarityLeft();

        for(uint i = imageInfo.length -1; i >= 0; i--){
          imageData memory image = imageInfo[i];
          bool canMint = amountMinted[i] < image.Max;
          if(canMint) {
            setting -= image.Rarity;
            if(value >= setting) {
              NFTInfo.push(NFTData(image.Name, image.Rarity, i));
              amountMinted[i] += 1;
              return;
            }
          }
  ...
  function getTotalRarity() internal view returns(uint256 total){
        total = 0;
        for(uint i=0; i<imageInfo.length; i++){
            imageData memory image = imageInfo[i];
            total +=  image.Rarity;
  ...
  function setMaxSupply(uint256 estTotalToAdd) external onlyOwner {

    uint256 actualAdded = 0;
    uint256 totalRarity = getTotalRarity();
    for(uint i=0; i < imageInfo.length; i++){
        imageData storage image = imageInfo[i];
        uint256 toAdd = estTotalToAdd * image.Rarity / totalRarity;
        image.Max += toAdd;
        actualAdded += toAdd;
    }
    
    maxSupply += actualAdded;
 ...
 function addFreeWhitelistUserOrAddMoreSpots(address _user, uint256 _howMany) public onlySub {
  ...
  function FGSetFGAdmin(address newFGAdmin) public onlyFG {
    ...
    function getTokenRarity(uint256 _tokenId) external view returns(uint256) {
        NFTData memory data = NFTInfo[_tokenId-1];
        return (data.Rarity);
    }
    ...
    function pageInfo() external view returns (uint256 _MaxSupply, uint256 _CurrentSupply, uint256 _MaxMint, address _PayToken, uint256 _CostBNB, uint256 _CostToken, bool _goPublic, address _owner, address _subOp) {
        return (maxSupply,totalSupply(),maxMintAmount,PayToken, costBNB, costToken, goPublic, owner(), subOperator);
    }
