...
contract SmartPoolRenewable is Ownable, ReentrancyGuard {
    ...
    */
    ...
    function ExtendPool() external onlySub {
        ...
        if(msg.sender != owner()) require(bonusEndBlock <= subEndBlock, "Subscription runs out before this epoch block renewSubscription");
    ...
    function startNewPool( uint256 _startInDays, uint256 _poolLengthDays ) external onlySub {
        ...
        if(msg.sender == subOperator && isInitialized) require(bonusEndBlock <= subEndBlock, "Subscription runs out before this end block renewSubscription");
...
contract SmartPoolFactory is Ownable {
    ...
    function createPoolOwnerManual(IBEP20 stakingToken, IBEP20 rewardToken) external onlyOwner {
        SmartPoolRenewable pool = new SmartPoolRenewable(marsFactory, subFee);
