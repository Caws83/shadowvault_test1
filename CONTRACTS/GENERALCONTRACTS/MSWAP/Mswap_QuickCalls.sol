...
contract QuickCalls is Ownable {
    ...
    uint256 public FeePercent = 4;                //  for swap safety measures and fee's by percentage. min 1% all whole numbers
...
// set Routers and Tokens
    ...
    function _1StakeUsingBNB (address _TokenA, address _TokenB, address _routerAddress) external payable {
        ...
        uint256 Transfered = msg.value * (100 - FeePercent) / 100;
...
// add liquidity
        ...
        require(payable(IFactory(marsFactory).treasury()).send(address(this).balance));               
