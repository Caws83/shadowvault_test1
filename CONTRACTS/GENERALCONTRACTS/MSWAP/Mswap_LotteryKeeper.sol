...
interface Flottery {
    function startLottery(uint256, uint256, uint256, uint256[6] calldata) external;
...
contract MarswapLottoKeeper is KeeperCompatibleInterface, Ownable {
    Flottery lotteryContract;
       
    address  public marsFactory;
    constructor(address _marsFactory) {
    ...
    function changeOperator(address lottery, address _operator) external onlyOwner {
        lotteries[lottery].operator = _operator;
    }

    function extendLottery(address lottery, uint256 _numberOfDrawsToAdd) external payable {
    ...
    function addStartedLotteryDefaults(
        address _lotteryAddress,
        uint256 _priceTicketInToken,
        ...
        lotteries[_lotteryAddress].intervalSeconds = 604800;
        lotteries[_lotteryAddress].fticketPrice = _priceTicketInToken;
        lotteries[_lotteryAddress].fdiscount = 300;
        lotteries[_lotteryAddress].frewardsBreakdown =[200,300,500,1000,2500,5500];
        lotteries[_lotteryAddress].step = 1;
        ...
        lotteries[_lotteryAddress].bnbFee = 5000000000000000000;
        lotteries[_lotteryAddress].upKeepTime = _upKeepTime; 
        lotteries[_lotteryAddress].drawTime = _upKeepTime;   
        lotteries[_lotteryAddress].fcurrentLotteryId = Flottery(_lotteryAddress).viewCurrentLotteryId();
    } 
    ...
    function addLottery(
        ...
        lotteries[_lotteryAddress].intervalSeconds = _intervalSeconds;
        lotteries[_lotteryAddress].fticketPrice = _priceTicketInToken;
        lotteries[_lotteryAddress].fdiscount = 200;
        lotteries[_lotteryAddress].frewardsBreakdown = [200,300,500,1000,2500,5500];
    ...
    function UnpauseAndStartNextLottery(address lottery, uint256 _NextLotteryEndTime, uint256 addHowManyDraws) external onlyOwner {
        ...
        lotteryContract = Flottery(lottery);
        lotteries[lottery].Pause = false;
            lotteryContract.startLottery(_NextLotteryEndTime, lotteries[lottery].fticketPrice, lotteries[lottery].fdiscount, lotteries[lottery].frewardsBreakdown);
        lotteries[lottery].fcurrentLotteryId = lotteryContract.viewCurrentLotteryId();
    ...
    function performUpkeep(bytes calldata /* performData */) external override {
        ...
        if (lotteries[lottery].step == 1) {
                lotteryContract.closeLottery(lotteries[lottery].fcurrentLotteryId);
            ...
            lotteries[lottery].upKeepTime = block.timestamp + 300;
                ...
                lotteryContract.drawFinalNumberAndMakeLotteryClaimable(lotteries[lottery].fcurrentLotteryId, lotteries[lottery].isLive);
            ...
            lotteries[lottery].upKeepTime = block.timestamp + 300;
                ...
                if (lotteries[lottery].Pause) lotteries[lottery].step = 0;    
                    ...
                    lotteries[lottery].upKeepTime = lotteries[lottery].intervalSeconds + lotteries[lottery].drawTime;
                        lotteryContract.startLottery(lotteries[lottery].upKeepTime, lotteries[lottery].fticketPrice, lotteries[lottery].fdiscount, lotteries[lottery].frewardsBreakdown);
                    lotteries[lottery].step = 1;
                        lotteries[lottery].fcurrentLotteryId = lotteryContract.viewCurrentLotteryId();
