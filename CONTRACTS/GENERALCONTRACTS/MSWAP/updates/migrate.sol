...
contract migrator is Ownable {
    ...
    uint256 MAX_INT = 2**256 - 1;
     ...
     constructor(address _bnbAddress) {
        bnbAddress = _bnbAddress;
    }

    function checkAllowance1(address _Token, uint256 _amount, address _router) private {
        if (IERC20(_Token).allowance(address(this),_router) < _amount && _Token != bnbAddress){
    ...
    function migrate(address _LPToken, uint256 _TokenInput, address _origRouter, address _newRouter) external {

        IERC20 TokenLP = IERC20(_LPToken);
        ...
        IRouter origRouter = IRouter(_origRouter);
        IRouter newRouter = IRouter(_newRouter);
        TokenLP.safeTransferFrom(address(msg.sender), (address(this)), _TokenInput);
        
        checkAllowance1(_LPToken, _TokenInput, _origRouter);
        
// Remove Liquidity to Tokens        
        if (Token1 == bnbAddress) {
            origRouter.removeLiquidityETHSupportingFeeOnTransferTokens(
            Token0,
            _TokenInput,
            1,
            1,
...
// Set post quantities
        uint256 t0Amount = balanceCheck(Token0);
        uint256 t1Amount = balanceCheck(Token1);    
        checkAllowance1(Token0, t0Amount, _newRouter);
        checkAllowance1(Token1, t1Amount, _newRouter);

        if (Token1 == bnbAddress) {
                newRouter.addLiquidityETH{value: t1Amount} (
                    Token0,
                    t0Amount,
                    0,
                    0,
        ...
        sendLeftOvers(Token0);
        sendLeftOvers(Token1);

    }

    function sendLeftOvers(address _Token) internal {
         if (_Token == bnbAddress){
                uint256 balance = address(this).balance;
