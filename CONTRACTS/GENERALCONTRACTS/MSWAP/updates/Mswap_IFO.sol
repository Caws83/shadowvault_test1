...
contract TokenSale is IIFOV2, ReentrancyGuard, Ownable {
    ...
    function FinalizeRound() external onlyOwner {
        ...
        // Hard Cap Hit - take marsFee and send the rest to dev wallet
            ...
            uint256 fee = (totalStaked * marsFee) / 1000;
            lpToken.safeTransfer(address(msg.sender), fee);
            lpToken.safeTransfer(DevWallet, lpToken.balanceOf(address(this)));     
        ...
        // take marsFee and send raised to dev wallet.  calculate and send remaining Offering tokens to Dev team
            ...
            uint256 tokensSold1 = _poolInformation[1].offeringAmountPool * _poolInformation[1].totalAmountPool / _poolInformation[1].raisingAmountPool;
    ...
    function setPoolsAfterSendingTokens(uint256 offering1, uint256 offering2, uint256 _raiseBasic, uint256 _raiseUnlimited, uint256 _basicUserLimit1) external onlyOwner {
        require(_raiseBasic + _raiseUnlimited > SoftCap, "combined raise limits must be more than softcap");

        setPool(offering1, _raiseBasic, _basicUserLimit1, false, 0);
        setPool(offering2, _raiseUnlimited, 0, false, 1);
        HardCap = _raiseUnlimited + _raiseBasic;
    }
