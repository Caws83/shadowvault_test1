...
contract TokenSale is IIFOV2, ReentrancyGuard, Ownable {
    ...
    function FinalizeRound() external {
        ...
        uint256 totalStaked = _poolInformation[0].totalAmountPool + _poolInformation[1].totalAmountPool;
        uint256 totalOffer = _poolInformation[0].offeringAmountPool + _poolInformation[1].offeringAmountPool;
        ...
        // Hard Cap Hit - take marsFee and send the rest to dev wallet
        } else if(totalStaked >= HardCap) {
            uint256 fee = (lpBalance * marsFee) / 1000;
            TransferHelper.safeTransferETH(treasury,fee);
            TransferHelper.safeTransferETH(devWallet,address(this).balance);
  
        // take marsFee and send raised to dev wallet.  calculate and send remaining Offering tokens to Dev team
        } else {
            uint256 tokensSold0 = 0;
            uint256 tokensSold1 = 0;
                     
            if(_poolInformation[0].offeringAmountPool > 0) tokensSold0 = (_poolInformation[0].offeringAmountPool * _poolInformation[0].totalAmountPool) / _poolInformation[0].raisingAmountPool;
            if(_poolInformation[1].offeringAmountPool > 0) tokensSold1 = (_poolInformation[1].offeringAmountPool * _poolInformation[1].totalAmountPool) / _poolInformation[1].raisingAmountPool;
            uint256 tokensLeftOver = totalOffer - (tokensSold0 + tokensSold1);
            offeringToken.safeTransfer(devWallet, tokensLeftOver);
    ...
    function setPoolsAfterSendingTokens(uint256 offering1_Amount, uint256 offering2_Amount, uint256 _raiseBasic_Goal, uint256 _raiseUnlimited_Goal, uint256 _basicUserLimit1) external onlyOwner {
        require(_raiseBasic_Goal + _raiseUnlimited_Goal > SoftCap, "combined raise limits must be more than softcap");

        if(offering1_Amount > 0) setPool(offering1_Amount, _raiseBasic_Goal, _basicUserLimit1, false, 0);
        setPool(offering2_Amount, _raiseUnlimited_Goal, 0, false, 1);
        HardCap = _raiseUnlimited_Goal + _raiseBasic_Goal;
    }
...
contract IFOFactory is Ownable {
    ...
    function createSale(
        ...
        address devWallet = IFactory(marsFactory).treasury();
        require(msg.value >= subFee, "Not enough Fee Paid");
        TransferHelper.safeTransferETH(devWallet,address(this).balance);
        // create Sale
         TokenSale sale = new TokenSale(_offeringToken, _devWallet, _startEpoch, _endEpoch, _softCap, marsFactory, marsFee);
        _tokenSales.push(sale);
        // send tokens for sale
        uint256 TokensForSale = basicOffering + unlimOffering;
        _offeringToken.transferFrom(msg.sender, address(sale), TokensForSale);
        // setup single pool
        sale.setPoolsAfterSendingTokens(basicOffering, unlimOffering, basicRaise, unlimRaise, basicLimit );
