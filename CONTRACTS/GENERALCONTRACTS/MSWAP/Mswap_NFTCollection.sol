...
contract MarSwapNFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint256 public FeePercent = 10;                 // Percentage of sales to MarSwap
  ...
  function rng() internal view returns(uint256) {
    uint256 seed = block.timestamp + block.difficulty + block.gaslimit + totalSupply() + gasleft(); 
  ...
  function setBaseURI(string memory _newBaseURI) public onlySub {
    baseURI = _newBaseURI;
  }
