...
// MARSWAP Renewable NFT staking Contract
...
contract SmartNFTPoolRenewable is IERC1155Receiver, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // treasury Subscription info
    uint256 public subFee =  50000000000000000000;    
    uint256 public subEndBlock = 0;
    uint256 public subLengthDays = 32;
    ...
    // The staked token
    IFarmageddonNFT public stakedToken;
    ...
    struct UserInfo {
        ...
        uint256[] tokenIds;
        uint256[] amountOfTokenId;       
    }

    function userInfo(address _user) external view returns(uint256 amount, uint256 rewardDebt, uint256[] memory tokenIds, uint256[] memory amountOfIds) {
        amount = usersInfo[_user].amount;
        rewardDebt = usersRewardDebt[_user][currentRound];
        tokenIds = usersInfo[_user].tokenIds;
        amountOfIds = usersInfo[_user].amountOfTokenId;
    }
    ...
    constructor(address _marsFactory, uint256 _subFee) {
   ...
   // Subscription
    ...
    function setSubFees(uint256 _subFee, uint256 _subLengthDays) external onlyOwner {
        subFee = _subFee;
    ...
    function RenewOrExtendSubscriptionSixWeeks() external payable onlySub {
        ...
        if(block.timestamp <= subEndBlock) subEndBlock += (subLengthDays * 86400);
        else subEndBlock = block.timestamp + (subLengthDays * 86400); 
    ...
    */
    ...
    function startNewPoolOrRound( uint256 _startInDays, uint256 _poolLengthDays ) external onlySub {
        ...
        if(msg.sender != owner() && isInitialized) require(bonusEndBlock <= subEndBlock, "Subscription runs out before this end block renewSubscription");
        
        // set new rewards per block based off the new information
        round.rewardPerBlock = totalNewReward / totalBlocks;
        if(subEndBlock == 0) {
            subEndBlock = block.timestamp + (subLengthDays * 86400);
            if(msg.sender == subOperator) require(_poolLengthDays <= subLengthDays);
        }
    ...
    function setNextRewardToken(IBEP20 _rewardToken) external onlySub {
        ...
        if(isRewardToken[address(rewardToken)]) {
            for(uint i=0; i < rewardTokenCount; i++) {
                if(roundInfo[i].rwdToken == address(_rewardToken)) currentRound = i;
            }
        } else {
            // NEW TOKEN
    ...
    function claimAll(uint256 amountAfter) internal {
        _updatePool();
        ...
        for (uint i = 0; i < rewardTokenCount; i++){
                ...
                usersRewardDebt[msg.sender][i] = amountAfter.mul(round.accTokenPerShare).div(round.PRECISION_FACTOR);
    ...
    // claims rewards
    function stake(uint256[] memory tokenIds, uint256[] memory amounts) public {
         UserInfo storage user = usersInfo[msg.sender];
         uint256 amountAfter = user.amount + tokenIds.length;
        claimAll(amountAfter);
    ...
    /// @notice Stake 1 or Multiple TokenId's. 
    ...
    */
    ...
    function claimPrevRewardsByRound(uint256 _round) public {
        _updatePool();
        UserInfo storage user = usersInfo[msg.sender];
        require(_round  < rewardTokenCount, "not a Round"); 
                RoundInfo storage round = roundInfo[_round];
                uint256 pending = user.amount.mul(round.accTokenPerShare).div(round.PRECISION_FACTOR).sub(usersRewardDebt[msg.sender][_round]);
            
                if (pending > 0) {
                    IBEP20 rwd = IBEP20(round.rwdToken);
                    try rwd.transfer(address(msg.sender), pending) {} catch {}
                    usersRewardDebt[msg.sender][_round] = user.amount.mul(round.accTokenPerShare).div(round.PRECISION_FACTOR);
                    round.prevAndCurrentRewardsBalance -= pending;
                }
    }

    function clearRewardsByRound(uint256 _round) public {
        _updatePool();
        UserInfo storage user = usersInfo[msg.sender];
        require(_round  < rewardTokenCount, "not a Round"); 
                RoundInfo storage round = roundInfo[_round];
                ...
                if (pending > 0) {
                    usersRewardDebt[msg.sender][_round] = user.amount.mul(round.accTokenPerShare).div(round.PRECISION_FACTOR);
                    if(round.prevAndCurrentRewardsBalance > pending) round.prevAndCurrentRewardsBalance -= pending;
                    else round.prevAndCurrentRewardsBalance = 0;
                }
    }
    ...
    function recoverTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
            ...
            require(block.timestamp > bonusEndBlock, "Pool Must be ended");
    ...
    function allPendingRewards(address _user) public view returns (address[] memory _rwdToken, uint256[] memory _amount, bool hasRewards) {
        UserInfo memory user = usersInfo[_user];
        ...
        for (uint i = 0; i < rewardTokenCount; i++) {
                ...
                if(i == currentRound) _amount[i] = pendingReward(_user);
                else _amount[i] = user.amount.mul(round.accTokenPerShare).div(round.PRECISION_FACTOR).sub(usersRewardDebt[_user][i]);
    ...
    function getUserPoolInfo(address _user) external view returns (
                ...
                bool _hasPrevRewards,
                uint256[] memory _rewards
                
                ) {
        UserInfo memory user = usersInfo[_user];
        _rewards = new uint256[](rewardTokenCount);
        (, _rewards , _hasPrevRewards) = allPendingRewards(_user);
        return (user.amount, user.tokenIds, pendingReward(_user), _hasPrevRewards, _rewards);
    }
    ...
    function getMainPoolInfo() external view returns (
        ...
        uint256 _endBlock
            ...
            RoundInfo memory round = roundInfo[currentRound];
            return (round.rewardPerBlock, totalStaked, startBlock, bonusEndBlock);  
        }
    ...
    function checkOtherNewRewards(uint256 _round) public view returns(uint256 totalNewReward){
            RoundInfo memory round = roundInfo[_round];
            IBEP20 token = IBEP20(round.rwdToken);
            totalNewReward = (token.balanceOf(address(this)) - round.prevAndCurrentRewardsBalance);    
    }

    function NEWOtherRewardWithdraw(uint256 _round) external onlyOwner {
        uint256 totalNewReward = checkOtherNewRewards(_round);
        RoundInfo memory round = roundInfo[_round];
        IBEP20 token = IBEP20(round.rwdToken);
        require(totalNewReward > 0, "No New Reward Tokens to Withdrawl");
        token.safeTransfer(address(msg.sender), totalNewReward);
    }
...
contract NFTPoolFactory is Ownable {
    ...
    event createdNFTPool(address Pool);

    SmartNFTPoolRenewable[] private _SmartPools;
    ...
    function createNFTPool(IFarmageddonNFT nftContract, IBEP20 partnerToken) external payable {
         address treasury = IFactory(marsFactory).treasury();
         require(payable(treasury).send(subFee));
            SmartNFTPoolRenewable pool = new SmartNFTPoolRenewable(marsFactory, subFee);
            _SmartPools.push(pool);
            SmartNFTPoolRenewable(pool).setupTokens(nftContract,partnerToken);
            SmartNFTPoolRenewable(pool).setSubOperator(msg.sender);
            SmartNFTPoolRenewable(pool).transferOwnership(Controller);
            
            emit createdNFTPool(address(pool));
    }

      function createNFTPoolAdmin(IFarmageddonNFT nftContract, IBEP20 partnerToken) external onlyOwner {
            SmartNFTPoolRenewable pool = new SmartNFTPoolRenewable(marsFactory, subFee);
            _SmartPools.push(pool);
            SmartNFTPoolRenewable(pool).setupTokens(nftContract,partnerToken);
            SmartNFTPoolRenewable(pool).transferOwnership(Controller);
      ...
      function removePool(SmartNFTPoolRenewable poolAddress) external onlyOwner  {
    ...
    function addPool(SmartNFTPoolRenewable poolAddress) external onlyOwner {
        _SmartPools.push(poolAddress);
    }
    
    
    function getPool(uint256 number) public view returns (SmartNFTPoolRenewable){
