...
contract MBiSwapRouter is IBiswapRouter02, Ownable {
    using SafeMath for uint;
    ...
    constructor(address _factory, address _WETH, address _treasury, uint256 _txFee) public {
        factory = _factory;
        WETH = _WETH;
        treasury = _treasury;
        txFee = _txFee;
        ...
        returns (uint[] memory amounts)
    {
        ...
        require(amounts[0] <= msg.value - fee, 'Router: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(BiswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
