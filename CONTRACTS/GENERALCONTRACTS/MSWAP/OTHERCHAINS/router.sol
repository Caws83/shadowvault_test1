...
library ILibrary {
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
        require(amountA > 0, 'ILibrary: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'ILibrary: INSUFFICIENT_LIQUIDITY');
...
contract othersRouter is IRouter02, Ownable {
    ...
    constructor(address _factory, address _WETH, address _treasury, uint256 _txFee) {
        factory = _factory;
        WETH = _WETH;
        treasury = _treasury;
        txFee = _txFee;
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = ILibrary.getReserves(factory, tokenA, tokenB);
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) public pure virtual override returns (uint amountB) {
        return ILibrary.quote(amountA, reserveA, reserveB);
    }
