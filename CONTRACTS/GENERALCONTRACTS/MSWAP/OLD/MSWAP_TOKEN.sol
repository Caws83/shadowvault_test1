...
contract MSWAPSwapper is ReentrancyGuard {
    ...
    function buyTokens(uint amountIn, address to) internal {
        ...
        amountIn = takeFee(amountIn, _FEE);
    ...
    function sellTokensForETH(uint amountIn, address to) external nonReentrant {
        ...
        address factory = _factory;
            ...
            takeFee(address(this).balance, _FEE);
...
contract MARSWAP is Ownable, ERC20('MARSWAP', 'MSWAP') {
    ...
    constructor(address _router, address _MarketingWallet, uint256 initialSupply, address _burnToken)  {
       devFee = 200;  // 100 = 1%
       burnFee = 200; // 100 = 1%
       swapperFee = 25; // 100 = 1%
    ...
    function setBurnToken(address _newBurnToken) external onlyOwner {
        require(_isContract(_newBurnToken), "Must be a Token");
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if(automatedMarketMakerPairs[to] || automatedMarketMakerPairs[from]) {
                uint256 extraFee;
                if(devFee >0 || burnFee >0) extraFee =(amount * devFee)/10000 + (amount * burnFee)/10000;
    ...
    function SwapFees() private lockTheSwap {
          ...
          address[] memory path = new address[](2);
            ...
            uint256 toBurn = ERC20(burnToken).balanceOf(address(this));
            totalBurned += toBurn;
            IERC20(burnToken).transfer(dead, toBurn);
