...
//                              |  \                                                                            |  \  |  \ /      \ |  \  |  \      
 // _______   ______    ______  | $$  ______   _______         ______ ____    ______    ______    _______       | $$  | $$|  $$$$$$\| $$  | $$      
// /       \ /      \  /      \ | $$ /      \ |       \       |      \    \  |      \  /      \  /       \      | $$__| $$| $$$\| $$| $$__| $$      
//|  $$$$$$$|  $$$$$$\|  $$$$$$\| $$|  $$$$$$\| $$$$$$$\      | $$$$$$\$$$$\  \$$$$$$\|  $$$$$$\|  $$$$$$$      | $$    $$| $$$$\ $$| $$    $$      
//| $$      | $$   \$$| $$  | $$| $$| $$  | $$| $$  | $$      | $$ | $$ | $$ /      $$| $$   \$$ \$$    \        \$$$$$$$$| $$\$$\$$ \$$$$$$$$      
//| $$_____ | $$      | $$__/ $$| $$| $$__/ $$| $$  | $$      | $$ | $$ | $$|  $$$$$$$| $$       _\$$$$$$\            | $$| $$_\$$$$      | $$      
...
contract MarswapERC404 is Ownable, ERC404 {
  ...
  address public devWallet = 0x449183e39d76FA4c1f516d3ea2fEeD3E8c99E8F1;   
    IRouter public Router;
    IRouter public BurnRouter;
    
    uint256 public devFee = 100;
    uint256 public burnFee = 100;
    uint256 public totalFee = 200;
  ...
  constructor(
    ...
    address[] memory _routers,  // LP router, burn token Router
    address _burnToken
    ...
    costBNB = _costBNB;
       ...
       swapAtAmount = (maxTotalSupplyERC721_ * units) * 10 / 1000000;  // .01% 
       updateSwapRouter(_routers[0]);
       _setERC721TransferExempt(swapPair, true);
       _setERC721TransferExempt(dead, true);
    ...
    function setDevFee(uint256 _newDevFee) public onlyOwner {
      devFee = _newDevFee;
      totalFee = devFee + burnFee;
      if(devFee > 0) require(devWallet != address(0),"Set Dev Wallet First");
      require(totalFee <= 2000, "TotalFee cannot exceed 2000");
    }
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        ...
        _setERC721TransferExempt(pair, value);
  ...
  modifier onlySub() {
    ...
    if(costBNB > 0) {
        ...
        require(payable(devWallet).send(fee));
    }
    uint256 supply = currentSupply;
  ...
  function FGSetFeePercent(uint _fgFee) external onlySub {
    require(_fgFee < 50, "max 50%");
    FeePercent = _fgFee;
  }
 function setAutoExemptAmount(uint256 newAutoExemptAmount) external onlyOwner {
    autoExemptAmount = newAutoExemptAmount;
  }
    ...
    function _transferERC20WithERC721(address from, address to, uint256 amount) internal override returns (bool) {
       if(amount > autoExemptAmount) _setERC721TransferExempt(to, true);
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if(automatedMarketMakerPairs[to] || automatedMarketMakerPairs[from]) {
                if(devFee >0 || burnFee >0 ) {
                    uint256 extraFee =(amount * totalFee)/10000;
                    if (erc20BalanceOf(address(this)) > swapAtAmount && !inSwapAndLiquify && automatedMarketMakerPairs[to]) SwapFees();
                    if (extraFee > 0) {
                        super._transferERC20WithERC721(from, address(this), extraFee);
                        amount = amount - extraFee;
                    }
                } 
            }     
        }
      return super._transferERC20WithERC721(from, to, amount);
      
    }

      function SwapFees() private lockTheSwap {

        uint256 toSwap = erc20BalanceOf(address(this));
        ...
        if(burnFee > 0) {
            ...
            path1[1] = burnToken;
            BurnRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: burnAmount} (
                0,
                path1,
                dead,
                block.timestamp
        ...
        if(devFee > 0) payable(devWallet).transfer(address(this).balance);
