...
contract UltimateToken is IERC20, Ownable {
    ...
    constructor(
        ...
        if(baseTokenForPair != mainRouter.WETH()){            
         ...
         _approve(address(this), address(swapCaller), MAX);
        ...
        isExcludedFromFee[_realOwner] = true;
        ...
        emit Transfer(address(0), _realOwner, _totalSupply);
    }
    ...
    function updateUniswapV2Pair(address _baseTokenForPair) external onlyOwner
    {
        ...
        mainPair = IUniswapV2Factory(mainRouter.factory()).getPair(address(this),baseTokenForPair);
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        ...
        mainPair = IUniswapV2Factory(mainRouter.factory()).getPair(address(this),baseTokenForPair);
        if(mainPair == address(0)){
            mainPair = IUniswapV2Factory(mainRouter.factory()).createPair(address(this), baseTokenForPair);
        }
        _approve(address(this), address(mainRouter), MAX);
        if(baseTokenForPair != mainRouter.WETH()){
            IERC20(baseTokenForPair).approve(address(mainRouter), MAX);            
