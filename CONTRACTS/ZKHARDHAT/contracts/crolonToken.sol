...
contract zkCrolon is Ownable, ERC20 ("zk Crolon Mars", "zkCLMRS") {
    ...
    constructor(
        ...
        setBurnToken(_burnToken);
        BurnRouter = IRouter(_burnRouter);

        setDevFee(_devFee);
        setMarketingFee(_marketingFee);
        setBurnFee(_burnFee);
    ...
    function setDevFee(uint256 _newDevFee) public onlyOwner {
      devFee = _newDevFee;
      totalFee = devFee + burnFee + liqFee + marketingFee;
    ...
    function setBurnToken(address _newBurnToken) public onlyOwner {
        burnToken = _newBurnToken;
        if(_newBurnToken == address(this)) isSelfBurn = true;
        else isSelfBurn = false;
    }
    function setBurnFee(uint256 _newBurnFee) public onlyOwner {
      ...
      if(burnFee > 0 ){
        ...
        if(!isSelfBurn) require(BurnRouter != IRouter(address(0)), "Burn Router not Set");
      }
    }
    ...
    function SwapFees() private lockTheSwap {
        ...
        if(isSelfBurn && burnFee > 0){
            uint256 toSelfBurn = contractTokenBalance * burnFee / totalFee;
            super._transfer(address(this), dead, toSelfBurn);
        ...
        if(!isSelfBurn && burnFee > 0) {
            uint256 burnAmount = address(this).balance * burnFee / totalFee;
        ...
        if(liqFee > 0) {
            ...
            uint256 bnbToAddLiquidityWith = (deltaBalance * liqFee) / (((totalFee - burnFee) *2) - liqFee);
        ...
        if(marketingFee > 0) {
            uint256 toMarketing = address(this).balance * marketingFee / (devFee + marketingFee);
            (bool success,) = marketingWallet.call{value:toMarketing}(new bytes(0));
            require(success, 'TransferHelper: ETH_TRANSFER_FAILED');
        }
        if(devFee > 0) {
             (bool success,) = devWallet.call{value:address(this).balance}(new bytes(0));
            require(success, 'TransferHelper: ETH_TRANSFER_FAILED');    
        }
