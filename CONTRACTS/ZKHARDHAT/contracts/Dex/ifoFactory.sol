...
contract TokenSale is IIFOV2, ReentrancyGuard, Ownable {
    ...
    function changemarsFee(uint256 _newmarsFee) external onlyOwner {
        require(block.timestamp < endBlock, "Cannot change after pools end");
        marsFee = _newmarsFee;
        require(_newmarsFee <= 50, "Fee to High");
    }

    function changeSocials(string memory _logo, string memory _website, string memory _bannerURI, string memory _telegramURI, string memory _XURI ) external {
        require(msg.sender == devWallet || msg.sender == owner(), "Must have access");
        logoURI = _logo;
        websiteURI = _website;
        bannerURI = _bannerURI;
        telegramURI = _telegramURI;
        XURI = _XURI;
    }

    function socials() public view returns (string memory _logo, string memory _website, string memory _bannerURI, string memory _telegramURI, string memory _XURI ){
        return ( logoURI, websiteURI, bannerURI, telegramURI, XURI);
    }
    ...
    function liquifyLockAndSend() internal returns (uint256 leftovers) {
        ...
        address lpReceiver = autoBurnLP ? 0x000000000000000000000000000000000000dEaD : lockDays > 0 ? address(this) : devWallet;
...
contract IFOFactoryV3 is Ownable {
    ...
    function createSale(
        IBEP20 _offeringToken,
        uint256[9] memory basicSale,
        address[2] memory addresses,
        bool burnLP,
        // uint256 basicOffering,       0
        // uint256 basicRaise,          1
        // uint256 basicLimit,          2
        // address _devWallet,          0
        // uint256 _startEpoch,         7
        // uint256 _endEpoch,           8
        // uint256 _softCap,            3
        // uint256 _percentToLiquidity, 4
        // uint256 _listingPrice,       5
        // uint256 _lockDays,           6
        // address _router,             1
        ...
        TransferHelper.safeTransferETH(treasury,address(this).balance);
         uint256 TokensForSale = basicSale[0];
         require(basicSale[4] < 100 && (basicSale[4] == 0 || basicSale[4] >= 51),"If over 0, must be more than 51%");
        // if set autoLiquify check to ensure listing price is lower than sale price
       if(basicSale[4] > 0){
            uint256 dec = _offeringToken.decimals();
            TokensForSale += getTokensforLiquidity( basicSale[1], basicSale[5], basicSale[4], dec);

       }

        // create Sale
         TokenSale sale = new TokenSale(_offeringToken, addresses[0], basicSale[7], basicSale[8], basicSale[3], treasury, marsFee, addresses[1], burnLP);
        _tokenSales.push(sale);
        // send tokens for sale
       
        _offeringToken.transferFrom(msg.sender, address(sale), TokensForSale);
        // setup single pool
        sale.setPoolsAfterSendingTokens(basicSale[0], basicSale[1], basicSale[2], basicSale[4], basicSale[5], basicSale[6] );
        sale.changeSocials(socials[0], socials[1], socials[2], socials[3], socials[4]);
    ...
    function changeSaleFee(uint256 newFee) external onlyOwner {
