...
library ILibrary {
    ...
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'ILibrary: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'ILibrary: ZERO_ADDRESS');
    }
...
contract QuickCalls is Ownable {
    ...
    uint256 public FeePercent = 10;     // 0.1%      
...
// set Routers and Tokens
    ...
    constructor(address _bnbAddress, address _treasury) {
        bnbAddress = _bnbAddress;
        treasury = _treasury;
        transferOwnership(treasury);
    }
    
    function _SetFeePercent(uint256 _FeePercentInBip) external onlyOwner {
        require(_FeePercentInBip >= 1 && _FeePercentInBip <= 1000, "Between 1 and 1000");
        FeePercent = _FeePercentInBip;
    }

    
    function getAmountOut(uint amountIn, address[] memory path, routerAddress usedRouter ) internal view returns (uint256) {
        // Step 1: Perform getAmountsOut with a smaller input amount to estimate price impact
        uint256 smallAmountIn = amountIn / 100; // Example: use 1% of the actual amountIn
        uint256[] memory smallAmountsOut = usedRouter.getAmountsOut(smallAmountIn, path);

        // Step 2: Calculate expected output without slippage
        uint256 expectedAmountOut = smallAmountsOut[1] * amountIn / smallAmountIn;

        // Step 3: Perform getAmountsOut with the actual input amount
        uint256[] memory amountsOut = usedRouter.getAmountsOut(amountIn, path);
        uint256 actualAmountOut = amountsOut[1];

        // Step 4: Calculate price impact
        uint256 priceImpact = (expectedAmountOut > actualAmountOut) 
            ? (expectedAmountOut - actualAmountOut) * 10000 / expectedAmountOut 
            : 0;

        // Step 5: Ensure price impact is within acceptable slippage
        require(priceImpact <= slippage, "Price impact exceeds slippage tolerance");

        // Calculate the amountOut with slippage
        uint256 amountOutMin = (actualAmountOut * (10000 - slippage)) / 10000;

        return amountOutMin;
    }

    
    function _1StakeUsingBNB (address _TokenA, address _TokenB, address _routerAddress, uint256 _slippage) external payable {
        router = _routerAddress;
        RA = routerAddress(router);
        if(_slippage != slippage) slippage = _slippage;
        setBNBBalance(bnbAddress, msg.value);
        uint256 InitialB = 0;
        uint256 InitialA = 0;
        uint256 swapAmount = 0;
        uint256 Transfered = msg.value * (10000 - FeePercent) / 10000;
    ...
    if ( _TokenB == bnbAddress ) {
        InitialB = balanceCheck(_TokenB) - Transfered;
      } else {
        InitialB = balanceCheck(_TokenB);
      }
    InitialA = balanceCheck(_TokenA);

        swapAmount = (Transfered / 10000 * (5000 - FeePercent));
        address[] memory path2 = new address[](2);
                    path2[0] = RA.WETH();
                    path2[1] = _TokenA;

                    uint256 amountOutMin = getAmountOut(swapAmount, path2, RA);
    ...
    if (_TokenB != bnbAddress ) {
       swapAmount = Transfered - swapAmount;
       address[] memory path21 = new address[](2);
                    path21[0] = RA.WETH();
                    path21[1] = _TokenB;
                    uint256 amountOutMin21 = getAmountOut(swapAmount, path21, RA);
                    RA.swapExactETHForTokensSupportingFeeOnTransferTokens{value: swapAmount} (
                    amountOutMin21,
                    path21,
                    address(this),
...
// add liquidity
        ...
        (bool success,) = treasury.call{value:address(this).balance}(new bytes(0));
        require(success, 'TransferHelper: ETH_TRANSFER_FAILED');
        BNBSTORED = address(this).balance;

    }

    function _1StakeToken (uint256 _TokenInput, address _TokenH, address _TokenA, address _TokenB, address _routerAddress, address _routerAddress2, uint256 _slippage) external {
        router = _routerAddress;
    ...
    setBNBBalance(_TokenH,_TokenInput);
...
// perform swap One is swap amount 0 ie. tokenH == tokenB skip this swap as we already have token B
        if (swapAmount > 0) {
                ...
                path[0] = _TokenH;
                path[1] = RA.WETH();
                uint256 amountOutMin = getAmountOut(swapAmount, path, RA);
...
// if tokenB is not BNB we will swap to Token B using ROUTER2
                if (_TokenB != bnbAddress){
                    ...
                    path2[0] = RA2.WETH();
                    path2[1] = _TokenB;
                    uint256 amountOutMin2 = getAmountOut(swapAmount, path2, RA2);
...
// Pre Swap 2 check for Token A
 ...
 // perform swap if Token H is Token A skipped to LP creation as we made the swap above.           
            if (_TokenH != _TokenA) {
// set swap amount
                swapAmount = (postSwap1 / 10000 * (5000 - FeePercent));
...
// perform swap
                if (_TokenB == bnbAddress) {
                    ...
                    path3[0] = RA2.WETH();
                    path3[1] = _TokenA;
                    uint256 amountOutMin3 = getAmountOut(swapAmount, path3, RA2);
                    RA2.swapExactETHForTokensSupportingFeeOnTransferTokens{value: swapAmount} (
                    amountOutMin3,
                    path3,
                    address(this),
                    ...
                    path4[0] = _TokenB;
                    path4[1] = RA2.WETH();
                    uint256 amountOutMin4 = getAmountOut(swapAmount, path4, RA2);
                    ...
                    path5[0] = RA2.WETH();
                    path5[1] = _TokenA;
                    uint256 amountOutMin5 = getAmountOut(swapAmount, path5, RA2);
                    RA2.swapExactETHForTokensSupportingFeeOnTransferTokens{value: swapAmount} (
                    amountOutMin5,
                    path4,
                    address(this),
...
// Function for Breaking LP and returning 1 Token
    function _BreakLPReturnToken (address _LPToken, uint256 _TokenInput, address _TokenA, address _TokenB, address _router, address _TokenH, address _router2, uint256 _slippage) external {
        RA = routerAddress(_router);
        RA2 = routerAddress(_router2);
        if(_slippage != slippage) slippage = _slippage;
        breakLPReturnToken(_LPToken, _TokenInput, _TokenA, _TokenB, _router, _TokenH);
    }

    function breakLPReturnToken (address _LPToken, uint256 _TokenInput, address _TokenA, address _TokenB, address _router, address _TokenH) internal {
        setBNBBalance(_LPToken, _TokenInput);
...
// Swap Token A to ETH
             if (_TokenA != _TokenH ) {
                ...
                path6[0] = _TokenA;
                path6[1] = RA.WETH();
                uint256 amountOutMin6 = getAmountOut(post1A, path6, RA);
                RA.swapExactTokensForETHSupportingFeeOnTransferTokens(
                post1A,
                amountOutMin6,
                path6,
...
// IF B is not BNB swap token B to BNB
            if (_TokenB != _TokenH) {
                if (_TokenB != bnbAddress) {
                    ...
                    path7[0] = _TokenB;
                    path7[1] = RA.WETH();
                    uint256 amountOutMin7 = getAmountOut(post1B, path7, RA);
                    RA.swapExactTokensForETHSupportingFeeOnTransferTokens(
                    post1B,
                    amountOutMin7,
                    path7,
     ...
     bnb2 = ((address(this).balance - BNBSTORED) * (10000 - FeePercent) / 10000 );
...
// if Output token is NOT bnb. swap to output token
           ...
           if (_TokenH == bnbAddress) {
                returnBNB(bnb2);
                    ...
                    path8[0] = RA.WETH();
                    path8[1] = _TokenH;
                    uint256 amountOutMin8 = getAmountOut(bnb2, path8, RA2);
    ...
    function setBNBBalance(address token, uint256 amount) public {
        if(token == bnbAddress) BNBSTORED = address(this).balance - amount;
        else BNBSTORED = address(this).balance;
    }

    function _bnbToToken (address _TokenOut, address _RouterOut, uint256 _slippage) external payable {
        _tokenToToken(bnbAddress, msg.value, _TokenOut, _RouterOut, _RouterOut, _slippage);
    }
// HARVEST AND SWAP TO A TOKEN w/Fee
    function _tokenToToken(address _TokenIn, uint256 _AmountIn, address _TokenOut, address _RouterIn, address _RouterOut, uint256 _slippage) public {
           require(_TokenIn != _TokenOut,"Token In cannot be the same as Token out");
           if(_slippage != slippage) slippage = _slippage;
            setBNBBalance(_TokenIn, _AmountIn);
            uint256 bnb2 = 0;
        ...
        TokenA = IERC20(_TokenIn);
            ...
            if (_TokenIn != bnbAddress) {
                    ...
                    path7[0] = _TokenIn;
                    path7[1] = RA.WETH();
                    address swapFactory = RA.factory();
                    uint256 amountOutMin7 = getAmountOut(_AmountIn, path7, routerAddress(_RouterIn));
...
// calls used in functions
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        internal
    {
        RA = routerAddress(swapRouter);
        address WETH = RA.WETH();
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, ILibrary.pairFor(swapFactory, path[0], path[1]), amountIn
        );
        _swapSupportingFeeOnTransferTokens(path, address(this), swapFactory, swapRouter);
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to, address swapFactory, address swapRouter) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = ILibrary.sortTokens(input, output);
            IPair pair = IPair(ILibrary.pairFor(swapFactory, input, output));
            (uint amount0Out, uint amount1Out) = getinfo(pair, input, token0, swapRouter);
            address to = i < path.length - 2 ? ILibrary.pairFor(swapFactory, output, path[i + 2]) : _to;
            pair.swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }

    function getinfo(IPair pair, address input, address token0, address swapRouter) internal view returns (uint amount0Out, uint amount1Out){
            (uint reserve0, uint reserve1,) = pair.getReserves();
            (uint reserveInput, uint reserveOutput) = input == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
            uint amountInput = IERC20(input).balanceOf(address(pair)) - (reserveInput);
            uint amountOutput = routerAddress(swapRouter).getAmountOut(amountInput, reserveInput, reserveOutput);
        ...
        function withdawlBNB() external onlyOwner {
            (bool success,) = msg.sender.call{value:BNBSTORED}(new bytes(0));
            require(success, 'TransferHelper: ETH_TRANSFER_FAILED');
            BNBSTORED = address(this).balance;
        }
