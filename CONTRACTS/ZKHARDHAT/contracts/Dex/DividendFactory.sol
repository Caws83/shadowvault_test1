...
// File: Factory/Working Files/Div-Factory-test.sol
...
interface LPFactory {
    function minPercentOfSupplyForDiv() external view returns (uint256);
...
contract LPDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("LP_Dividend_Tracker", "LP_Dividend_Tracker") {}
    ...
    function getminimumTokenBalanceForDividends() public view returns (uint256) {
        uint256 min = LPFactory(factory).minPercentOfSupplyForDiv();
        return totalSupply() * min / 10000000; // must have minimum % of supply to get rewards
    }
    ...
    function canAutoClaim(uint256 lastClaimTime) private view returns (bool) {
        uint256 claimWait = LPFactory(factory).claimWait();
    ...
    function setBalance(address payable account, uint256 newBalance) external onlyOwner {
    	...
    	if(newBalance >= getminimumTokenBalanceForDividends()) {
    ...
    function process(uint256 gas, uint256 howMany) public onlyOwner returns (uint256, uint256, uint256) {
    	...
    	while(claims <= howMany && gasUsed < gas && iterations < numberOfTokenHolders) {
    		_lastProcessedIndex++;
...
contract DividendFactory {
    LPDividendTracker dividendTracker;
    mapping (address => address) public DividenTrackerForLP;

    function createTrackerFromLP(address factory, address token0, address token1) external returns (address newTracker) {
        
        // create dividend tracker etc.
        bytes memory bytecode = type(LPDividendTracker).creationCode;
         bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            newTracker := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        
       dividendTracker = LPDividendTracker(payable(newTracker));

        dividendTracker.initialize(factory);
        ...
        dividendTracker.transferOwnership(msg.sender);
        DividenTrackerForLP[msg.sender] = address(dividendTracker);
