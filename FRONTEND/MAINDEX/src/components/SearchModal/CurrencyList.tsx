...
import { Currency, CurrencyAmount, currencyEquals, getETHER, Token } from 'sdk'
...
function currencyKey(currency: Currency, chainId: number): string {
  return currency instanceof Token ? currency.address : currency === getETHER(chainId) ? 'ETHER' : ''
}
...
export default function CurrencyList({
  ...
  const itemData: (Currency | undefined)[] = useMemo(() => {
    let formatted: (Currency | undefined)[] = showETH ? [getETHER(chainId), ...currencies] : currencies
  ...
  const Row = useCallback(
    ({ data, index, style }) => {
      ...
      const handleSelect = () => onCurrencySelect(currency, chainId)
      ...
      if (showImport && token) {
        return (
          <ImportRow style={style} token={token} showImportView={showImportView} setImportToken={setImportToken} dim chainId={chainId} />
        )
      }
      return (
        <CurrencyRow
