...
export interface EasySelectHostProps {
  options: Host[]
  startIndex?: number
  selectedId?: number
  onChange?: (option: Host) => void
}

const EasySelectHost: React.FunctionComponent<EasySelectHostProps> = ({
  options,
  startIndex,
  selectedId,
  onChange,
      ...
      {containerSize.width !== 0 && (
        <DropDownHeader onClick={toggling}>
          <Flex flexDirection="row" style={{ width: '100%' }}>
            <TokenImage
              token={options[selectedOptionIndex]?.payoutToken}
              host={hosts.marswap}
              chainId={options[selectedOptionIndex]?.chainId}
              width={24}
              height={24}
