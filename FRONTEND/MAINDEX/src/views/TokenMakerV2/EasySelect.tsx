...
import { TokenImageString } from 'components/TokenImage'
      ...
      {containerSize.width !== 0 && (
        <DropDownHeader onClick={toggling}>
          <Flex flexDirection="row" style={{ width: '100%' }}>
            <TokenImageString
              source={options[selectedOptionIndex]?.address ?? getWrappedAddress(chainId)}
              width={24}
              height={24}
          ...
          {options.map((option, index) =>
            index !== selectedOptionIndex ? (
              <ListItem onClick={onOptionClicked(index)} key={option?.symbol}>
                <Flex>
                <TokenImageString
                    source={option?.address ?? getWrappedAddress(chainId)}
                    width={24}
                    height={24}
                    style={{ marginRight: '18px' }}
                  />
