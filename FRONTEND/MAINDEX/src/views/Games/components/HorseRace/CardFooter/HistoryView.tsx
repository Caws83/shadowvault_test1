        ...
        for (let i = eventsWith.length - 1; i >= 0; i--) {
          if (personal === 1 && account) {
            if (eventsWith[i].args[0] === account) {
              ...
              if (new BigNumber(eventsWith[i].args[3].toString()).lt(2)) {
                w += 1
                wA = new BigNumber(eventsWith[i].args[4].toString()).plus(wA)
              } else {
                l += 1
                lA = new BigNumber(eventsWith[i].args[4].toString()).plus(lA)
              }
            }
          } else {
            e.push(eventsWith[i].args)
