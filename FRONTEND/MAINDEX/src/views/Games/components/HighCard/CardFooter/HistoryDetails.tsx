...
const BetDetails: React.FC<BetDetailsProps> = ({ game, round }) => {
  ...
  const [isWinner] = useState<boolean>(round[5])
  const [amount] = useState<BigNumber>(new BigNumber(round[5] ? round[6].toString() : round[8].toString()))
  const [lastSuitInfo] = useState<number>(new BigNumber(round[1].toString()).toNumber())
  const [lastCardInfo] = useState<number>(new BigNumber(round[2].toString()).toNumber())
  const [newSuitInfo] = useState<number>(new BigNumber(round[3].toString()).toNumber())
  const [newCardInfo] = useState<number>(new BigNumber(round[4].toString()).toNumber())
  const [jackPot] = useState<boolean>(round[7])
        ...
        <StyledBetDetails>
          <Flex alignItems="center" justifyContent="space-between" mb="8px">
            ...
            <Flex alignItems="center">
              ...
              {jackPot && `${headerIcon} JACKPOT!`}
            </Flex>
          </Flex>

          <Flex alignItems="center" justifyContent="space-between" mb="16px">
            <Text color="secondary">Previous:</Text>
            {getCoinImage(lastSuitInfo, lastCardInfo)}
          </Flex>
          <Flex alignItems="center" justifyContent="space-between" mb="16px">
            <Text color="secondary">New Card:</Text>
            {getCoinImage(newSuitInfo, newCardInfo)}
          </Flex>

          <ActionContainer>
            <Flex alignItems="center" justifyContent="space-between" mb="16px">
              <Text color="secondary">Potential:</Text>
