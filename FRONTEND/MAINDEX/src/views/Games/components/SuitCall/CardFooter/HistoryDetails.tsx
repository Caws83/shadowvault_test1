...
const BetDetails: React.FC<BetDetailsProps> = ({ game, round }) => {
  ...
  const [isWinner] = useState<boolean>(round[4])
  const [amount] = useState<BigNumber>(new BigNumber(round[5].toString()))
  const [houseSuitInfo] = useState<number>(new BigNumber(round[2].toString()).toNumber())
  const [houseCardInfo] = useState<number>(new BigNumber(round[1].toString()).toNumber())
  const [suitChoice] = useState<number>(new BigNumber(round[3].toString()).toNumber())
