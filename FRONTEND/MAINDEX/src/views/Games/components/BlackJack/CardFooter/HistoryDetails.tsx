...
const BetDetails: React.FC<BetDetailsProps> = ({ game, round }) => {
  ...
  const [playerSuits] = useState<BigNumber[]>(round[1])
  const [playerCards] = useState<BigNumber[]>(round[2])
  const [houseSuits] = useState<BigNumber[]>(round[3])
  const [houseCards] = useState<BigNumber[]>(round[4])
  const [amount] = useState<BigNumber>(new BigNumber(round[5].toString()))
  const [playerTotal] = useState<BigNumber>(new BigNumber(round[6].toString()))
  const [houseTotal] = useState<BigNumber>(new BigNumber(round[7].toString()))
  const [isWinner] = useState<boolean>(round[9])
  ...
  const getCoinImage = (suits: BigNumber[], cards: BigNumber[]) => {
    return (
      <Flex flex="1" flexDirection="row" mr={['8px', 0]}>
        {cards.map((x, index) => {
          return (
            <CardHand
              suit={suits[index].toNumber()}
              card={cards[index].toNumber()}
              height={50}
              width={50}
              folder={folder}
            />
      ...
      {isOpen && (
        <StyledBetDetails>
          ...
          <Flex alignItems="center" justifyContent="space-between" mb="16px">
            <Text color="secondary">{`Player: ${playerTotal.toNumber()}`} </Text>
            {getCoinImage(playerSuits, playerCards)}
          </Flex>
          <Flex alignItems="center" justifyContent="space-between" mb="16px">
            <Text color="secondary">{`House: ${houseTotal}`}</Text>
            {getCoinImage(houseSuits, houseCards)}
          </Flex>
