...
import ResultLowRollerModal from '../modals/resultLowRollerModal'
...
const GameAction: React.FC<{
  ...
  const { maxBetAmount, quickBetAmount, userData, payoutRate, payToken, multipliers2, lowRoller, folder } = game

  const { onStartLR, onRollLR, onRollLRD } = useLowRoller(game)
  ...
  const [onPresentResults] = useModal(<ResultLowRollerModal game={game} />, false, true, 'LowRoll')

  const houseTotal = new BigNumber(lowRoller.houseDice1.toString()).plus(new BigNumber(lowRoller.houseDice2.toString()))
  ...
  const onClickStart = async () => {
    setPlaying(true)
    onPresentResults()
    await onStartLR(betAmount.toString(), diceChoice)
    setPlaying(false)
  }

  const onClickRollLR = async () => {
    setPlaying(true)
    onPresentResults()
    await onRollLR()
    setPlaying(false)
    ...
    return (
      <>
                ...
                <Flex flex="1" flexDirection="row" mr={['8px', 0]}>
                  <Flex flex="1" flexDirection="column" mr={['8px', 0]}>
                    <Heading as="h1" scale="xl" color="secondary" mb="50px" textAlign="center">
                      <DiceImage2
                        size={new BigNumber(lowRoller.diceChoice.toString()).toNumber()}
                        number={new BigNumber(lowRoller.houseDice1.toString()).toNumber()}
                        height={50}
                        width={50}
                        folder={folder}
                      />
                  ...
                  <Flex flex="1" flexDirection="column" mr={['8px', 0]}>
                    <Heading as="h1" scale="xl" color="secondary" mb="50px" textAlign="center">
                      <DiceImage2
                        size={new BigNumber(lowRoller.diceChoice.toString()).toNumber()}
                        number={new BigNumber(lowRoller.houseDice2.toString()).toNumber()}
                        height={50}
                        width={50}
                        folder={folder}
                      />
        ...
        {!lowRoller.isGameStarted ? (
          ...
          <>
            <Button style={{ marginTop: '15px' }} onClick={onClickRollLR}>
              ! Roll Dice !
            </Button>
            <Button style={{ marginTop: '15px' }} onClick={onClickRollLRD}>
              ! DOUBLE !
            </Button>
          </>
        )}
