...
const AprRow: React.FC<AprRowProps> = ({ pool, stakedBalance, performanceFee = 0 }) => {
  ...
  const tooltipContent = isAutoVault
    ? t('APY includes compounding, APR doesn’t. This pool’s MSWAPF is compounded automatically, so we show APY.')
  ...
  const apyModalLink = stakingToken.address ? `/swap?outputCurrency=${getAddress(stakingToken.address, pool.chainId)}` : '/swap'
  ...
  return (
    ...
    <Text fontSize="12px">CHAIN:</Text>
    <Text fontSize="12px"> {t('%chain%', { chain: client?.chain?.name })}</Text>
    </Flex>
    <Flex alignItems="center" justifyContent="space-between">
      <Text fontSize="12px">{isAutoVault ? `${t('APY')}:` : `${t('APR')}:`}</Text>
      {isFinished || !apr ? (
