      ...
      {containerSize.width !== 0 && (
        <DropDownHeader onClick={toggling}>
          <Flex flexDirection="row" style={{ width: '100%' }}>
            <CurrencyLogo chainId={options[selectedOptionIndex]?.chainId} currency={options[selectedOptionIndex]} size="24px" style={{ marginRight: '20px' }} />
          ...
          {options.map((option, index) =>
            index !== selectedOptionIndex ? (
              <ListItem onClick={onOptionClicked(index)} key={option?.name}>
                <Flex>
                  <CurrencyLogo chainId={option?.chainId} currency={option} size="24px" style={{ marginRight: '100px' }} />
