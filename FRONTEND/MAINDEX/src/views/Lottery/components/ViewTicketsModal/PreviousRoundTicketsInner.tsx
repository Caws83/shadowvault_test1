...
const PreviousRoundTicketsInner: React.FC<{ roundId: string; lotteryToken: Token; lottoPrice: BigNumber }> = ({
  roundId,
  lotteryToken,
  lottoPrice,
}) => {
  ...
  const { totalTickets } = useGetUserLotteryGraphRoundById(roundId, lotteryToken)
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const userTickets = await fetchUserTicketsForOneRound(account, roundId, lotteryToken, l.chainId)
      const lotteryData = await fetchLottery(lotteryToken, roundId, l.chainId)
    ...
    fetchData()
  }, [roundId, account, totalTickets, lotteryToken])
  ...
  return (
    <>
          ...
          allUserTickets.map((ticket) => {
            return (
              <TicketNumber
                ...
                lotteryToken={ticket.lotteryToken}
              />
            )
          })
        ) : (
      ...
      <Flex borderTop={`1px solid ${(theme as any).colors.cardBorder}`} alignItems="center" justifyContent="center">
