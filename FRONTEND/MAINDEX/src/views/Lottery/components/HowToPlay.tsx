...
import { useLotteryPricePerTicket, useLotteryRewardsBreakdown, useLotteryTreasuryFee } from 'state/lottery/hooks'
...
export const PoolAllocations: React.FC<{ lotteryToken: Token }> = ({ lotteryToken }) => {
  const { t } = useTranslation()

  const rewardsBreakdown = useLotteryRewardsBreakdown(lotteryToken)
  const treasuryFee = useLotteryTreasuryFee(lotteryToken)
  const [rewardsPercent, setRewardsPercent] = useState([0, 0, 0, 0, 0, 0, 0])
  useEffect(() => {
    const percents = [0, 0, 0, 0, 0, 0, 0]
    const treasuryPercent = (treasuryFee / 10000) * 100
    const percentLeft = 100 - treasuryPercent
    for (let i = 0; i < 6; i++) {
      const percentReward = rewardsBreakdown[i] / 10000
      percents[i] = Math.floor(percentLeft * percentReward)
    }
    percents[6] = treasuryPercent
    setRewardsPercent(percents)
  }, [rewardsBreakdown, treasuryFee])
...
const HowToPlay: React.FC<{ lotteryToken: Token }> = ({ lotteryToken }) => {
  const { t } = useTranslation()
  const pricePerTicket = useLotteryPricePerTicket(lotteryToken)

  const steps: Step[] = [
    {
      label: t('Step %number%', { number: 1 }),
      title: t('Buy Tickets'),
      subtitle: t(`Price per ticket is set at ${pricePerTicket} ${lotteryToken.symbol} per ticket.`),
    },
    {
      label: t('Step %number%', { number: 2 }),
      title: t('Wait for the Draw'),
      subtitle: t('Check the Timer for your Desired lottery for Draw Times.'),
    },
    {
        ...
        <Flex flex="2" flexDirection="column">
          ...
          <BulletList>
            <li>
              <Text display="inline" color="textSubtle">
                {t(
                  `100% of the ${lotteryToken.symbol} paid by people buying tickets that round goes back into the prize pools.`,
                )}
              </Text>
            </li>
          </BulletList>
          ...
          <BulletList>
            <li>
              <Text display="inline" color="textSubtle">
                {t(
                  `After every round, if nobody wins in one of the prize brackets, the unclaimed ${lotteryToken.symbol} for that bracket rolls over into the next round and are redistributed among the prize pools.`,
                )}
              </Text>
            </li>
          </BulletList>
          <Heading my="16px" scale="md">
            {t(`${lotteryToken.symbol} Injections`)}
          </Heading>
          <BulletList>
            <li>
              {lotteryToken.symbol === tokens.mswap.symbol ? (
                <Text display="inline" color="textSubtle">
                  {t(
                    `Randomly ${lotteryToken.symbol} from the treasury is added to a lottery round. The injections willl be collected from Sources throughout MARSWAP including the Burn Pool from Lottery, Auto ${lotteryToken.symbol} Pool, and More. This ${lotteryToken.symbol} is of course also included in rollovers!`,
                  )}
                </Text>
              ) : (
                <Text display="inline" color="textSubtle">
                  {t(
                    `Randomly ${lotteryToken.symbol} from the Dev Team is added to a lottery round. The Token's Dev Team will inject to the lottery as they see fit. This ${lotteryToken.symbol} is of course also included in rollovers!`,
                  )}
                </Text>
              )}
            </li>
          ...
          <Heading my="16px" scale="md">
            {t('Burn / Dev Pool is Used in 2 ways:')}
          </Heading>
          <BulletList>
            <li>
              <Text display="inline" color="textSubtle">
                {t(`1: The ${lotteryToken.symbol} can be Burned Completely.`)}
              </Text>
            </li>
            <li>
              <Text display="inline" color="textSubtle">
                {t(
                  `2: The ${lotteryToken.symbol} from the Burn/Dev Pool can be split between MARSWAP and the Token Dev team. MARSWAP will use their portion to renew/extend the FG/MSWAPF Earn ${lotteryToken.symbol} Pools, and the Token Dev team can use their portion as they please. We recommend either burning or Filling their Stake ${lotteryToken.symbol} Earn ${lotteryToken.symbol} Pools.`,
                )}
              </Text>
            </li>
          </BulletList>
