...
describe('transaction reducer', () => {
  ...
  describe('addTransaction', () => {
    it('adds the transaction', () => {
      ...
      expect(txs[ChainId.PUPNET]).toBeTruthy()
      expect(txs[ChainId.PUPNET]?.['0x0']).toBeTruthy()
      const tx = txs[ChainId.PUPNET]?.['0x0']
  ...
  describe('finalizeTransaction', () => {
    ...
    it('sets receipt', () => {
      ...
      const tx = store.getState()[ChainId.PUPNET]?.['0x0']
      expect(tx?.summary).toEqual('hello world')
  ...
  describe('clearAllTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.PUPNET), String(ChainId.PUPNET)])
      expect(Object.keys(store.getState()[ChainId.PUPNET] ?? {})).toEqual(['0x0'])
      expect(Object.keys(store.getState()[ChainId.PUPNET] ?? {})).toEqual(['0x1'])
      store.dispatch(clearAllTransactions({ chainId: ChainId.PUPNET }))
      expect(Object.keys(store.getState())).toHaveLength(2)
      expect(Object.keys(store.getState())).toEqual([String(ChainId.PUPNET), String(ChainId.PUPNET)])
      expect(Object.keys(store.getState()[ChainId.PUPNET] ?? {})).toEqual([])
      expect(Object.keys(store.getState()[ChainId.PUPNET] ?? {})).toEqual(['0x1'])
    })
  })
})
