import { Currency, CurrencyAmount, getETHER, JSBI, Pair, Percent, Price, TokenAmount } from 'sdk'
...
export function useDerivedMintInfo(
  ...
  // pair
  const [pairState, pair] = usePair(dex, currencies[Field.CURRENCY_A], currencies[Field.CURRENCY_B])
  ...
  const dependentAmount: CurrencyAmount | undefined = useMemo(() => {
    ...
    if (independentAmount) {
      ...
      if (tokenA && tokenB && wrappedIndependentAmount && pair) {
        ...
        return dependentCurrency === ETHER ? CurrencyAmount.ether(dependentTokenAmount.raw, chainId) : dependentTokenAmount
      }
      return undefined
    }
    return undefined
