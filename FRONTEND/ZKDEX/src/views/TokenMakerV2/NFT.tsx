...
import { abi, byteCode } from 'config/abi/tokens/nftCollection'
...
import artifact from 'config/abi/tokens/MarSwapNFT.json'
...
const Nft: React.FC = () => {
  ...
  const constructSolidityUrl = () => {
    ...
    const solidityFileUrl = `${mainUrl}/source/nftCollection.sol`;
  ...
  const onClickConfirm = async () => {
    ...
    const encodedConstructorArgs = abiCoder.encode(
        ["string", "string","string", "string", "uint256", "uint256", "address", "uint256", "address"],
        constructorArgs
    );
      // verify contract
      ...
      tokens.push({ symbol: symbol, contractAddress: receipt.contractAddress, chain: chain.name, type: 'NFT'});
...
<Flex flexDirection={isMobile ? "column" : "row"}>
    <Flex flexDirection="row" flexWrap="wrap" justifyContent="center">
        ...
        <Tile style={{ width: isMobile ? '47%' : '197px' }}>
          <Flex flexDirection="column" minWidth="50%">
            <Flex alignItems="flex-start">
              <Text color={noSupply ? "failure" : "textSubtle"}>{t('NFT MAx Suplply:')}</Text>
            </Flex>
            <NumberInput
...
<ActionContainer>    
  <BorderContainer>
    ...
    {tokens.map((newToken) => (
      newToken.chain === chain.name && newToken.type === 'NFT' &&
