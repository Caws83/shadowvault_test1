...
const HasSharesActions: React.FC<HasStakeActionProps> = ({ pool, stakingTokenBalance, performanceFee }) => {
  ...
  const { cakeAsBigNumber, cakeAsNumberBalance } = convertSharesToCake(new BigNumber(userShares), new BigNumber(pricePerFullShare))
  const frtprice = useHostPricesBusd()[tokens.mswap.symbol]
  const stakedDollarValue = frtprice.gt(0)
    ? getBalanceNumber(cakeAsBigNumber.multipliedBy(frtprice), stakingToken.decimals)
    : 0
