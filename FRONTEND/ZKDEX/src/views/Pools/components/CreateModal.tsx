...
import { poolFactoryAbi } from 'config/abi/PoolFactory'
...
const CreateModal: React.FC<CreateModalProps> = ({ onDismiss }) => {
  ...
  const isOwner = account === getAddress(contracts.farmWallet, chainId)
      ...
      const { data } = useReadContract({
        abi: poolFactoryAbi,
        address: getAddress(contracts.poolFactoryV3, chainId),
        functionName: 'subFee',
        chainId
      })
  ...
  const TooltipComponent = () => (
    <>
      <Text mb="16px">{t('Use this to Create a New Token Staking Pool!')}</Text>
      <Text mb="16px">{t('Creating a Token Staking Pool will add it to the list Below.( Community Pools )')}</Text>
  ...
  const handleConfirmClick = async () => {
    ...
    try {
      ...
      const { request } = await simulateContract(config, {
        abi: poolFactoryAbi,
        address: getAddress(contracts.poolFactoryV3, chainId),
        functionName: isOwner ? 'createPoolOwner' : 'createPool',
  ...
  const handleChangeQueryStake = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setStakeIsToken(await isToken(event.target.value))
  }
