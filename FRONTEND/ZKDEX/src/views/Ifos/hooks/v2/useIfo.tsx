...
export const useFinalizeRound = (ifo: Ifo) => {
  ...
  const handleFinalize = useCallback(async () => {
    try {
      const { request } = await simulateContract(config, { abi: ifoV2Abi, address, functionName: 'FinalizeRound', chainId: ifo.dex.chainId })
  ...
  const handleEndEarly = useCallback(async () => {
    try {
      const { request } = await simulateContract(config, { abi: ifoV2Abi, address, functionName: 'endEarly', chainId: ifo.dex.chainId })
      const hash = await writeContract(config, request)
      const receipt = (await waitForTransactionReceipt(config, {hash})) as TransactionReceipt
