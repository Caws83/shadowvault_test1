...
import { useQuickHighRollerUserDataAsync } from 'state/games/hooks'
...
const ResultHRModal: React.FC<ResultModalProps> = ({ onDismiss, game }) => {
  ...
  const { onRollHR, onRollHRD } = useHighRoller(game)
  ...
  useQuickHighRollerUserDataAsync(account)
  ...
  useEffect(() => {
    if (didIWin && !loading) delay(showConfetti, 100)
  }, [didIWin, loading])
