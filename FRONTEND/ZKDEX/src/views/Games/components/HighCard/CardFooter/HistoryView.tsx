...
const HistoryView: React.FC<HistoryViewProps> = ({ game, eventsWith, winAmounts }) => {
  ...
  useEffect(() => {
    ...
    async function fetch() {
      if (winAmounts) {
        ...
        for (let i = winAmounts.length - 1; i >= 0; i--) {
          if (personal === 1 && account) {
            if (winAmounts[i].args[0] === account) {
              w += 1
              wA = new BigNumber(winAmounts[i].args[1].toString()).plus(wA)
            }
          } else w += 1
          wA = new BigNumber(winAmounts[i].args[1].toString()).plus(wA)
        }
        ...
        for (let i = eventsWith.length - 1; i >= 0; i--) {
          if (personal === 1 && account) {
            if (eventsWith[i].args[0] === account) {
              ...
              if (!eventsWith[i].args[5]) {
                l += 1
                lA = new BigNumber(eventsWith[i].args[8].toString()).plus(lA)
              }
            }
          } else {
            e.push(eventsWith[i].args)
    ...
    fetch()
  }, [account, personal, eventsWith, winAmounts])
