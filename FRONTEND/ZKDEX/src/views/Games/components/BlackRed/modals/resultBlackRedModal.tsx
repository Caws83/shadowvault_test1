...
const ResultBlackRedModal: React.FC<ResultModalProps> = ({ onDismiss, game }) => {
  ...
  const [isCardBlack, setIsCardBlack] = useState<boolean>()
  ...
  const [folder] = useState<string>(game.folder)
    ...
    const unwatch = useWatchContractEvent({
      ...
      eventName: 'blackOrRedResults',
      onLogs(logs) {
        logs.forEach((info) => {
        
            const { amount, isWinner, houseCard, houseSuit, isBlack } = info.args
            unwatch?.()
            setDidIWin(isWinner)
                ...
                <Flex flex="1" justifyContent="center" flexDirection="column" mr={['8px', 0]}>
                  <Flex flex="1" flexDirection="column" mr={['8px', 0]}>
                    ...
                    <Heading as="h1" scale="xl" color="secondary" mb="50px" textAlign="center">
                      <DeckCard suit={isCardBlack ? 1 : 0} card={0} height={50} width={75} folder={game.folder} />
                    </Heading>
                    <Heading as="h1" scale="xl" color="secondary" mb="75px" textAlign="center">
                      Drawn Card
                    </Heading>
...
export default ResultBlackRedModal
