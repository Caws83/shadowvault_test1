        ...
        for (let i = eventsWith.length - 1; i >= 0; i--) {
          if (personal === 1 && account) {
            if (eventsWith[i].player === account) {
              e.push(eventsWith[i])
              if (new BigNumber(eventsWith[i].result.toString()).eq(0)) {
                w += 1
                wA = new BigNumber(eventsWith[i].ammount.toString()).plus(wA)
              } else if (new BigNumber(eventsWith[i].result.toString()).eq(1)) {
                l += 1
                lA = new BigNumber(eventsWith[i].ammount.toString()).plus(lA)
              } else t += 1
            }
          } else {
            e.push(eventsWith[i])
    ...
    <>
      <ExpandedWrapper flexDirection="column">
        ...
        <Flex flexDirection="row" justifyContent="space-between">
          <Text color={wins > losses ? 'green' : 'red'}>{`${(wins / (losses + ties + wins)) * 100} % Win Ratio`} </Text>
