...
import { currencyEquals, Token, getWBONE } from './token'
...
function wrappedAmount(currencyAmount: CurrencyAmount, chainId: ChainId): TokenAmount {
  ...
  return new TokenAmount(WBONE[chainId], currencyAmount.raw)
}

function wrappedCurrency(currency: Currency, chainId: ChainId): Token {
  const WBONE = getWBONE()
  if (currency instanceof Token) return currency
  return WBONE[chainId]
}
...
export class Trade {
   ...
   */
  public static exactIn(route: Route, amountIn: CurrencyAmount, dex: FactoryInfo, chainId: number): Trade {
    return new Trade(route, amountIn, TradeType.EXACT_INPUT, dex, chainId)
  }
   ...
   */
  public static exactOut(route: Route, amountOut: CurrencyAmount, dex: FactoryInfo, chainId: number): Trade {
    return new Trade(route, amountOut, TradeType.EXACT_OUTPUT, dex, chainId)
  }

  public constructor(route: Route, amount: CurrencyAmount, tradeType: TradeType, dex: FactoryInfo, chainId: number) {
   ...
   */
  public minimumAmountOut(slippageTolerance: Percent, chainId: number): CurrencyAmount {
   ...
   * @param slippageTolerance tolerance of unfavorable slippage from the execution price of this trade
   */
  public maximumAmountIn(slippageTolerance: Percent, chainId: number): CurrencyAmount {
    invariant(!slippageTolerance.lessThan(ZERO), 'SLIPPAGE_TOLERANCE')
