...
const RoiCalculatorModal: React.FC<RoiCalculatorModalProps> = ({
  ...
  const displayConversionValue = `${Number(conversionValue).toLocaleString(undefined, {
    ...
    >
      <ScrollableContainer>
        <Flex flexDirection="column" mb="8px">
          ...
          <BalanceInput
            currencyValue={`${displayConversionValue} ${conversionUnit}`}
          ...
          <Flex justifyContent="space-between" mt="8px">
            ...
            <Button
              disabled={!stakingTokenBalance.gte(0) || !account}
              scale="xs"
              p="4px 16px"
              width="128px"
              variant="tertiary"
              onClick={() =>
                setPrincipalFromUSDValue(
                  getBalanceNumber(
                    new BigNumber(stakingTokenBalance.toString()).multipliedBy(stakingTokenPrice),
                  ).toString(),
                )
              }
            >
