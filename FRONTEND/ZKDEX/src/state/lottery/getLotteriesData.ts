...
export const MAX_LOTTERIES_REQUEST_SIZE = 10
...
const getLotteriesData = async (currentLotteryId: string, lotteryToken: Token, chainId: number): Promise<LotteryRoundGraphEntity[]> => {
  const idsForNodesCall = getRoundIdsArray(currentLotteryId)
  const nodeData = await fetchMultipleLotteries(lotteryToken, idsForNodesCall, chainId)
